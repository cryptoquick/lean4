...
/-!
# Systems.IntervalSet (Systems Lean)

Fixed-capacity **ordered U32 interval set** over caller-owned `lo`/`hi` slots —
interval-set-**shaped** dense ordered intervals, not a concurrent interval tree, not a
...
* Invariant: intervals in `[0, n)` are ordered by **strictly increasing** `lo`
  (unique left endpoints; set of intervals)

Simplified “interval set”: this port keeps a **single dense ordered leaf** (binary
lower-bound on `lo` + shift insert) with interval-set-shaped API names. Tree nodes,
augments, and concurrent stabbing structures are intentionally omitted for residual-clean
dual-param freestanding.
...
* **Not claimed:** concurrent interval sets, multi-overlap reporters, open/half-open
  product APIs, or `Std` interval-set parity.

## Intentional TCB (IntervalSet-local)

`U32.load`/`store`, `USize.mul`/`half` are freestanding `@[extern]` axioms
kept **here**. Name-pinned on ComplianceCorpus (`path name=Ident`).
-/

namespace Systems.IntervalSet
