...
/-!
# Systems.Splay (Systems Lean)

Fixed-capacity **splay-shaped ordered U32 map** over caller-owned key/value slots —
not a concurrent splay tree, not amortized rotation product, not malloc-backed nodes.
...
+ shift insert/remove) with splay-shaped API names. Zig-zag rotations, access-path
splaying, and concurrent structures are intentionally omitted for residual-clean
...
* **Not claimed:** full amortized splay rotations, concurrent maps, or `Std.TreeMap` parity.

## Intentional TCB (Splay-local)
