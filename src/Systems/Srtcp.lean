...
/-!
# Systems.Srtcp (Systems Lean)

SRTCP/RTCP-family **min-4** common header parse over dual caller `addr`/`len` byte
views — not a full SRTCP stack, not AES-CM/HMAC crypto product, not compound RTCP
walk, not RTP media.

Wire layout (RFC 3550 / RFC 3711-shaped; minimum **4** header bytes, network byte
order). SRTCP reuses the RTCP fixed header; this module is an **export-isolated**
RTCP-family twin of `Rtcp` for product consumers that name the SRTCP path.
...
**Distinct from `Rtcp` / `Srtp`:** same min-4 V/P/RC/PT/length field layout
as Rtcp, but distinct exports (`lean_fs_srtcp_*` vs `lean_fs_rtcp_*`). Srtp
is the min-12 RTP-family path (`lean_fs_srtp_*`). **Not full SRTCP crypto** (no
encrypt/decrypt, no auth tag, no E-bit/index product, no key derivation).
...
* **Min-4 SRTCP/RTCP-family header offsets only** — no compound packet walk, no
  SR/RR block product, no SDES/BYE/XR, **not full SRTCP crypto**. Distinct from
  Rtcp control path and Srtp min-12 media path.
...
/-- SRTCP/RTCP-family common header minimum length (4). -/
...
contributes a nonzero bit). Does not inspect body past the min fence. Not full SRTCP
crypto. -/
