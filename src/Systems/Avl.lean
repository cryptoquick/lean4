...
/-!
# Systems.Avl (Systems Lean)

Fixed-capacity **AVL-shaped ordered U32 map** over caller-owned key/value slots —
not a full concurrent AVL, not `Std.TreeMap`, not malloc-backed nodes.
...
+ shift insert/remove) with AVL-shaped API names. Multi-node rotations, parent links,
and height fields are intentionally omitted for residual-clean dual-param freestanding.
...
* **Not claimed:** full AVL rotations, concurrent maps, or `Std.TreeMap` parity.

## Intentional TCB (Avl-local)
