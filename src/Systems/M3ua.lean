...
/-!
# Systems.M3ua (Systems Lean)

M3UA-**shaped** minimum common header parse over dual caller `addr`/`len` byte views —
not a full SS7/SIGTRAN stack, not ASPSM/ASPTM state product, not payload walk.

Wire layout (RFC 4666-shaped; minimum **8** common-header bytes, network byte order):
...
* **Min-8 common header fields only** — no parameter TLVs, no routing context, no
  ASP/AS state machine, not an M3UA client/server.
...
/-- Minimum M3UA common header length (8). -/
...
Shaped for raw BE dwords in/after the M3UA common header. -/
