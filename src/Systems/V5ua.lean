...
/-!
# Systems.V5ua (Systems Lean)

V5UA-**shaped** minimum common header parse over dual caller `addr`/`len` byte views —
not a full SS7/SIGTRAN stack, not V5 User Adaptation state product, not payload walk.
Distinct from `M3ua` (M3UA MTP3 User Adaptation), `Sua` (SUA SCCP User
Adaptation), and `Iua` (IUA ISDN User Adaptation): this is V5 User Adaptation
common-header layout only (RFC 3807-shaped version/reserved/class/type/length over SCTP).

Wire layout (RFC 3807-shaped; minimum **8** common-header bytes, network byte order):
...
* **Min-8 common header fields only** — no parameter TLVs, no routing key, no
  ASP/AS state machine, not a V5UA client/server, not V5.2 user data product.
...
/-- Minimum V5UA common header length (8). -/
...
Shaped for raw BE dwords in/after the V5UA common header. -/
