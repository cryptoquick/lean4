...
/-!
# Systems.CacheIndex (Systems Lean)

Fixed-capacity **rebuild-skip cache-index** ordered unique `U32` key set over
caller-owned key slots — Lake/Slake **cache-index shaped** (presence index), twin of
`Trace` for "is this cache entry present". Not Lake cache monadic IO, not a
content-addressed store, and not crypto.

**Distinct from `Trace`:** same dense ordered unique U32 key/hash set model
and `mixPut` via `Hash.hashBytes`; this module uses CacheIndex-shaped API names
(`indexAt` twin of `traceAt` / `pageAt`). Residual-green product.
...
* Invariant: `keys[0..n)` is sorted **strictly increasing** (unique keys/hashes; set)

Ops:

* **init** / **clear** — occupancy-only reset helpers (caller sets `n := 0`)
* **lowerBound** / **contains** / **get** / **indexAt** — ordered lookup
* **insert** / **put** — insert a precomputed `U32` key (idempotent if present)
* **mixPut** — `Systems.Hash.hashBytes` over dual `addr`/`len`, then insert
* **remove** — drop a key when present
...
* **LP64 product harness:** `get`/`indexAt` miss uses `USize.neg1`. On ILP32 that collides
  with storing `UINT32_MAX` — treat as **LP64-only** (Map `get` class).
* **Not claimed:** Lake cache monadic IO, content-addressed store, or crypto hashes.

Honesty:

* **Rebuild-skip index shaped only** — not Lake cache IO, not content-addressed store,
...
* **LP64 product harness:** `get`/`indexAt` miss uses `USize.neg1`.

## Intentional TCB (CacheIndex-local)
...
/-- Cache-index entry at index as `USize`, or miss if `i ≥ n`. **LP64 only**.

CacheIndex-shaped name for ordered dense leaf slot access (twin of `get` /
Trace `traceAt` / PageSet `pageAt`). -/
public unsafe def indexAt (keys : USize) (n : USize) (i : USize) : USize :=
  bifUSize (USize.blt i n)
    (USize.ofU32 (U32.load (u32Slot keys i)))
    USize.neg1
...
/-- Insert precomputed key `key`. `0` ok; `3` if full / `cap == 0` / `n > cap`.
...
/-- Alias of `insert` (cache-index put of a precomputed key/hash). -/
