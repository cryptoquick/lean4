...
/-!
# Systems.Icmp (Systems Lean)

ICMP-**shaped** header field parse over dual caller `addr`/`len` byte views — not a
full ICMP stack, not an echo server, not IP encapsulation, not checksum verify.

Wire layout (RFC 792-shaped; minimum **8** header bytes, network byte order):
...
* **Header offsets only** — no ICMP type dispatch product, no echo reply generation,
  no checksum algorithm / verification, no IPv4/IPv6 envelope, no raw sockets.
