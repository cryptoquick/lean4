...
/-!
# Systems.BTreeMap (Systems Lean)

Fixed-capacity **B-tree-map-shaped ordered U32 map** over caller-owned key/value slots —
not a multi-node concurrent B+tree, not page-split product, not malloc-backed nodes.
...
+ shift insert/remove) with B-tree-map-shaped API names. Multi-level nodes, sibling
pointers, page splits, and concurrent structures are intentionally omitted for
...
* **Not claimed:** multi-node B+tree, concurrent maps, disk pages, or `Std.TreeMap` parity.

## Intentional TCB (BTreeMap-local)
