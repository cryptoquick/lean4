...
/-!
# Systems.Ice (Systems Lean)

ICE/STUN-family **connectivity-check**-shaped min-20 common header parse over dual
caller `addr`/`len` byte views — not a full ICE candidate gather/check stack, not
attribute walk, not TURN ChannelData product, not Binding-only (`Stun`) or
Allocate-only (`Turn`) peers.

Wire layout (RFC 8445 / RFC 5389-shaped; minimum **20** header bytes, network byte
order). ICE connectivity checks ride STUN Binding framing (`0x0001` method class),
with the same magic cookie and transaction-id layout as STUN/TURN family peers:
...
* **Min-20 STUN-family ICE connectivity-check header offsets only** — no ICE candidate
  gather, no checklist state machine, no attribute TLV walk, not an ICE agent.
  Distinct exports (`lean_fs_ice_*`) from `Stun` (`lean_fs_stun_*`) and
  `Turn` (`lean_fs_turn_*`).
...
/-- ICE STUN-family common header minimum length (20). -/
...
ICE connectivity checks use Binding (`0x0001`); Allocate-shaped TURN methods are
also readable (STUN family framing). -/
