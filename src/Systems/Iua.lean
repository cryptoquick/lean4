...
/-!
# Systems.Iua (Systems Lean)

IUA-**shaped** minimum common header parse over dual caller `addr`/`len` byte views —
not a full SS7/SIGTRAN stack, not ISDN User Adaptation state product, not payload walk.
Distinct from `M3ua` (M3UA MTP3 User Adaptation) and `Sua` (SUA SCCP User
Adaptation): this is ISDN User Adaptation common-header layout only (RFC 4233-shaped
version/reserved/class/type/length).

Wire layout (RFC 4233-shaped; minimum **8** common-header bytes, network byte order):
...
* **Min-8 common header fields only** — no parameter TLVs, no routing key, no
  ASP/AS state machine, not an IUA client/server, not Q.931 user data product.
...
/-- Minimum IUA common header length (8). -/
...
Shaped for raw BE dwords in/after the IUA common header. -/
