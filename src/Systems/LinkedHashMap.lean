...
/-!
# Systems.LinkedHashMap (Systems Lean)

Fixed-capacity **linked-hash-map-shaped ordered U32 map** over caller-owned key/value
slots — not a concurrent LRU, not open-addressing resize/heap product, not
`java.util.LinkedHashMap` parity.
...
Simplified “linked hash map”: this port keeps a **single dense ordered leaf** (binary
lower-bound + shift insert/remove) with linked-hash-map-shaped API names. Insert-order
doubly-linked lists, access-order LRU eviction, tombstones, load-factor policy, and
concurrent structures are intentionally omitted for residual-clean dual-param freestanding
(sibling of `HashMap` / `BTreeMap` dense leaves).
...
* **Not claimed:** insert-order linked list, concurrent LRU, resize heap, or classic
  `LinkedHashMap` parity. Dense key order is the residual-clean twin of `HashMap` / `OrderedMap`.

## Intentional TCB (LinkedHashMap-local)
...
namespace Systems.LinkedHashMap
