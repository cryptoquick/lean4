...
/-!
# Systems.SkipListMap (Systems Lean)

Fixed-capacity **skip-list-map-shaped ordered U32 map** over caller-owned key/value
slots — not a multi-level concurrent skip list, not probabilistic towers, not
malloc-backed nodes.
...
Simplified “skip list”: this port keeps a **single dense ordered leaf** (binary
lower-bound + shift insert/remove) with skip-list-map-shaped API names. Multi-level
towers, probabilistic promotion, and concurrent structures are intentionally omitted
...
* **Not claimed:** multi-level concurrent skip list, probabilistic towers, or
  `Std.TreeMap` parity.

## Intentional TCB (SkipListMap-local)
