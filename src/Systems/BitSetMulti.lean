...
/-!
# Systems.BitSetMulti (Systems Lean)

Fixed-capacity **multi-word U32 bitset** over caller-owned word slots — named multi-word
twin of `BitMap` / multi-word complement of single-word `BitSet`. Residual-clean
BitSet-shaped API. Not a growable infinite bit vector, not concurrent, not managed storage.
...
* Not claimed: infinite bits, popcount API, concurrent bitsets, or Init `BitSet` parity.
* Store status is dataflow-used so EmitC cannot DCE word writes (`never_extract`).

## Intentional TCB (BitSetMulti-local)
...
/-- Zero all `cap` bitset words. `0` ok. -/
