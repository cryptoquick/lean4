...
/-!
# Systems.OrderedU32Set (Systems Lean)

Fixed-capacity **ordered unique `U32` set** over caller-owned key slots — dense sorted
array with binary lower-bound insert/remove. Keys only (not key→value; see
`OrderedMap` for pairs). Not a concurrent set, not a tree-balanced set, not a
growable allocator-backed collection.
...
* **get** / **orderedAt** — key at index as `USize`, or miss
...
Freestanding dual-param contract (no multi-field product returns).
...
* **LP64 product harness:** `get`/`orderedAt` miss uses `USize.neg1`. On ILP32 that collides
  with storing `UINT32_MAX` — treat as **LP64-only** (Map `get` class).
* **Not claimed:** concurrent ordered sets, tree-balanced sets, or
  Java `TreeSet` / `ConcurrentSkipListSet` parity.
* **C ABI:** product exports stay `lean_fs_orderedset_*` (stable short name; not renamed).

## Intentional TCB (OrderedU32Set-local)
...
namespace Systems.OrderedU32Set
...
/-- Ordered-slot key at index as `USize`, or miss if `i ≥ n`. **LP64 only**.

OrderedSet-shaped name for ordered dense leaf slot access (twin of `get`). -/
public unsafe def orderedAt (keys : USize) (n : USize) (i : USize) : USize :=
  bifUSize (USize.blt i n)
    (USize.ofU32 (U32.load (u32Slot keys i)))
    USize.neg1
