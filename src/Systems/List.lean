...
/-!
# Systems.List (Systems Lean)

Fixed-capacity **dense list** over raw memory — freestanding fixed-cap U32 list helpers,
not Init/`Std` managed `List`, not GC cons cells / RC storage.
...
Ops (Vector sibling under a List product id):
...
* **isEmpty** / **isFull** / **clear** — occupancy helpers
...
* **Not claimed:** growable lists, Init/`Std` `List` parity, intrusive GC cons cells, concurrency, or managed storage.

## Intentional TCB (List-local)

`U32.load`/`store`, `USize.ofU32`/`mul` are freestanding `@[extern]` axioms kept
**here** (Vector/Stack parity). Name-pinned on ComplianceCorpus (`path name=Ident`).
-/
