...
/-!
# Systems.Webm (Systems Lean)

WebM/EBML-**shaped** header magic + element ID/size class scan over dual caller
`addr`/`len` byte views — not a full WebM/Matroska demux, not codec/track product,
not cluster/block walk.
...
* **Magic + ID/size VINT class only** — no nested element walk, no DocType string
  match, no cluster/block demux, not a WebM/Matroska product.
