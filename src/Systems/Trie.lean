...
/-!
# Systems.Trie (Systems Lean)

Fixed-capacity **trie-shaped ordered U32 map** over caller-owned key/value slots —
not a concurrent radix tree, not concurrent, not malloc-backed nodes.
...
+ shift insert/remove) with trie-shaped API names. Prefix edges, node arrays, and
compressed Patricia edges are intentionally omitted for residual-clean dual-param freestanding.
...
* **Not claimed:** concurrent radix, concurrent trees, or `Std.TreeMap` parity.

## Intentional TCB (Trie-local)
