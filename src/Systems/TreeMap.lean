...
/-!
# Systems.TreeMap (Systems Lean)

Fixed-capacity **ordered U32→U32 map** over caller-owned key/value slots — dense leaf
twin of `OrderedMap` / `BTreeMap` / `HashMap` — not a concurrent tree map,
not heap nodes, not multi-node tree product.
...
* **Not claimed:** concurrent tree map, red-black/B+ multi-node heap nodes, or resize.
  Dense key order is the residual-clean twin of `OrderedMap` / `BTreeMap` /
  `HashMap`.

## Intentional TCB (TreeMap-local)
