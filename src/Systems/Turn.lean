...
/-!
# Systems.Turn (Systems Lean)

TURN-**shaped** min-20 STUN-family common header parse over dual caller `addr`/`len`
byte views — not a full TURN/ICE stack, not attribute walk, not ChannelData demux
product, not a STUN Binding-only peer (`Stun`).

Wire layout (RFC 5766 / RFC 5389-shaped; minimum **20** header bytes, network byte
order). TURN Allocate/Refresh/ChannelBind ride the same STUN framing as Binding, with
TURN-shaped method codes (e.g. Allocate `0x0003`):
...
* **Min-20 STUN-family TURN header offsets only** — no TURN attribute TLV walk, no
  ICE candidate gather, no ChannelData 4-byte path, not a TURN client/server.
  Distinct exports (`lean_fs_turn_*`) from `Stun` (`lean_fs_stun_*`).
...
/-- TURN STUN-family common header minimum length (20). -/
...
TURN-shaped methods include Allocate (`0x0003`), Refresh (`0x0004`), etc.; Binding
(`0x0001`) is also readable (STUN family). -/
