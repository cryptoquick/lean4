...
/-!
# Systems.RbTree (Systems Lean)

Fixed-capacity **red-black-shaped ordered U32 map** over caller-owned key/value slots —
not a multi-node red-black tree, not concurrent, not malloc-backed nodes.
...
+ shift insert/remove) with red-black-shaped API names. Node colors, rotations, and
parent/child links are intentionally omitted for residual-clean dual-param freestanding.
...
* **Not claimed:** multi-node red-black, concurrent trees, or `Std.TreeMap` parity.

## Intentional TCB (RbTree-local)
      ...
      (let pos := lowerBound keys n key
       bifU32 (USize.blt pos n)
         (bifU32 (U32.beq (U32.load (u32Slot keys pos)) key)
           -- overwrite existing (allowed when full: n == cap, key present)
