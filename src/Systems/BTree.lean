...
/-!
# Systems.BTree (Systems Lean)

Fixed-capacity **B-tree-shaped ordered U32 map** over caller-owned key/value slots —
not a full B+tree database, not concurrent, not malloc-backed nodes.
...
+ shift insert/remove). Multi-level B-tree nodes, sibling pointers, and page splits
are intentionally omitted for residual-clean dual-param freestanding.
...
* **Not claimed:** full B+tree, variable fanout, disk pages, concurrent maps, or
  `Std.TreeMap` parity.

## Intentional TCB (BTree-local)
...
/-- Insert `key → val`. `0` ok; `3` if full / `cap == 0` for a **new** key.

If `key` already present, overwrites value and returns `0` without growing `n`.
    ...
    (let pos := lowerBound keys n key
     bifU32 (USize.blt pos n)
       (bifU32 (U32.beq (U32.load (u32Slot keys pos)) key)
         ...
         -- insert before pos (key < keys[pos]); need free slot
