...
/-!
# Systems.Srtp (Systems Lean)

SRTP/RTP-family **min-12** common header parse over dual caller `addr`/`len` byte
views — not a full SRTP stack, not AES-CM/HMAC crypto product, not MKI/auth-tag
walk, not RTCP compound product.

Wire layout (RFC 3550 / RFC 3711-shaped; minimum **12** header bytes, network byte
order). SRTP reuses the RTP fixed header; this module is an **export-isolated**
RTP-family twin of `Rtp` for product consumers that name the SRTP path.
...
**Distinct from `Rtp`:** same min-12 V/P/X/CC/M/PT/seq/timestamp/SSRC field
layout, but distinct exports (`lean_fs_srtp_*` vs `lean_fs_rtp_*`). Not full SRTP
crypto (no encrypt/decrypt, no auth tag, no MKI, no key derivation).
...
* **Min-12 SRTP/RTP-family header offsets only** — no CSRC list walk, no extension
  header product, no payload type registry, **not full SRTP crypto**. Distinct from
  Rtp media path and Rtcp min-4.
...
/-- SRTP/RTP-family common header minimum length (12). -/
