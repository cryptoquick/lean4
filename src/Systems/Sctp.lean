...
/-!
# Systems.Sctp (Systems Lean)

SCTP-**shaped** common header field parse over dual caller `addr`/`len` byte views —
...
## Intentional TCB (Sctp-local)

Header size, field offsets, and big-endian shift amounts are freestanding `@[extern]`
axioms kept **here**. Name-pinned on ComplianceCorpus (`path name=Ident`). Byte loads
reuse `Scalars`/`Bytes`; shift/or reuse `Numerics`.
-/

namespace Systems.Sctp
