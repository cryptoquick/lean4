...
/-!
# Systems.OrderedMap (Systems Lean)

Fixed-capacity **ordered U32→U32 map** over caller-owned key/value slots — dense leaf
twin of `BTreeMap` / `HashMap` / `LinkedHashMap` — not a concurrent ordered
map, not heap resize product, not multi-node tree.
...
* **Not claimed:** concurrent ordered map, red-black/B+ multi-node heap, or resize heap.
  Dense key order is the residual-clean twin of `HashMap` / `BTreeMap` /
  `LinkedHashMap`.

## Intentional TCB (OrderedMap-local)
