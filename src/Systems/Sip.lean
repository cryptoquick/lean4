...
/-!
# Systems.Sip (Systems Lean)

SIP-**shaped** first-line method / URI / version scan over dual caller `addr`/`len`
byte views — not a full SIP stack, not dialog/transaction state, not SDP/body product.

Request-line shape (RFC 3261-shaped; ASCII; line ends with `\n` or `\r\n`):

```
METHOD SP Request-URI SP SIP/x.y
```

Minimum fence is **8** bytes (shaped lower bound for a tiny `M U S/2.0`-class line).
...
* **hasVersion** — `1` if third token starts with `SIP/`, else `0`
* **versionOff** — offset of third token when `hasVersion`, else miss

Honesty:

* **First-line method/URI/version offsets only** — no header walk, no Via/Contact,
  no INVITE/REGISTER state machine, not a SIP client/server.
...
/-- SIP request-line shaped minimum length (8). -/
...
/-- `1` if bytes at `i` start `SIP/` (needs `i + 4 ≤ n`, i.e. width `fourUSize`). -/
public unsafe def isSipSlash (addr : USize) (i : USize) (n : USize) : U32 :=
  bifU32 (USize.blt n (USize.add i fourUSize)) U32.zero
    (bifU32 (U32.beq (loadAt addr i) cS)
      (bifU32 (U32.beq (loadAt addr (USize.add i off1)) cI)
        (bifU32 (U32.beq (loadAt addr (USize.add i off2)) cP)
          (bifU32 (U32.beq (loadAt addr (USize.add i off3)) cSlash) U32.one U32.zero)
          U32.zero)
        U32.zero)
      U32.zero)
...
contributes a nonzero bit). Does not require `SIP/` match. -/
           ...
           bifUSize (USize.blt sp2 n)
             (bifUSize (U32.beq (loadAt addr sp2) cSp)
               (let vo := USize.add sp2 USize.one
                bifUSize (U32.beq (isSipSlash addr vo n) U32.one) vo USize.neg1)
               USize.neg1)
             USize.neg1)))

/-- `1` if third token starts with `SIP/`, else `0`. -/
