...
/-!
# Systems.Smtp (Systems Lean)

SMTP-**shaped** first-line command / reply scan over dual caller `addr`/`len` byte
views — not a full SMTP stack, not dialog/session state, not MIME/body product.

First-line shapes (RFC 5321-shaped; ASCII; line ends with `\n` or `\r\n`):
...
Minimum fence is **4** bytes (shaped lower bound for a tiny `250\n` / `HELO`-class line).
...
* **First-line method/code + optional text offsets only** — no multi-line `250-`
  continuations product, no AUTH/STARTTLS state machine, not an SMTP client/server.
...
/-- Start of optional text: after `NNN ` / `NNN-` for replies, or after method SP.
