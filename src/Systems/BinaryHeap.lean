...
/-!
# Systems.BinaryHeap (Systems Lean)

Fixed-capacity **binary min-heap** of `U32` over caller-owned slots — heap-**shaped**
priority queue, not a concurrent heap allocator, not a growable `PriorityQueue`.
...
* **Capacity contract:** `n ≤ cap`; `u32Slot` uses `USize.mul i 4` without overflow checks
  (Vector parity). Keep `cap` small enough that `cap * 4` fits in `size_t`.
...
* **Not claimed:** growable heaps, max-heap, BST/TreeMap, concurrency, malloc-backed
  allocator, or managed storage. Distinct export ABI from `Tree`.
* **C ABI:** product exports stay `lean_fs_heap_*` (stable short name; not renamed to
  `lean_fs_binaryheap_*`).

## Intentional TCB (BinaryHeap-local)

`U32.load`/`store`, `USize.ofU32`/`mul`/`div`/`two` are freestanding `@[extern]` axioms
kept **here** (Tree/Vector parity). Name-pinned on ComplianceCorpus
(`path name=Ident`). Store status is dataflow-used so EmitC cannot DCE slot writes.
-/

namespace Systems.BinaryHeap
...
/-- Insert `val` at index `n` then sift-up. `0` ok; `errBounds` if full / `cap == 0` / `n > cap`.
