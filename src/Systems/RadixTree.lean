...
/-!
# Systems.RadixTree (Systems Lean)

Fixed-capacity **radix-tree-shaped ordered unique U32 set** over caller-owned key
slots — dense leaf twin of `BTreeSetLite` / `TreeMap` but **keys only** (no
vals buffer). Not a concurrent radix tree, not multi-node heap nodes, not a
prefix-trie product over byte paths.
...
+ shift insert/remove) with radix-tree-shaped API names. Multi-level nodes, bit
prefix strips, sibling pointers, and concurrent structures are intentionally omitted
for residual-clean dual-param freestanding.

Ops:

* **lowerBound** — first index `i` with `keys[i] ≥ key`, or `n` if all smaller
* **contains** — presence of `key`
* **get** / **keyAt** — key at index as `USize`, or miss
...
* **LP64 product harness:** `get`/`keyAt` miss uses `USize.neg1`. On ILP32 that collides
  with storing `UINT32_MAX` — treat as **LP64-only** (Map `get` class).
* **Not claimed:** multi-node radix / Patricia trie, concurrent sets, disk pages, or
  classic radix-tree path lookup parity.

## Intentional TCB (RadixTree-local)
