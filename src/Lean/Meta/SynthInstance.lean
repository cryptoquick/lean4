...
def synthInstanceCore? (type : Expr) (maxResultSize? : Option Nat := none) : MetaM (Option Expr) := do
  ...
  withInTypeClassResolution do
    ...
    | none =>
      ...
      recordMissDoneHeartbeats kind result?.isSome blkBits blkNone hb0
      match normCtx? with
      | none   => cacheResult cacheKey kind abstResult? result?
      | some c =>
        ...
        | some (nAbstResult?, nResult?) => cacheResult cacheKey kind nAbstResult? nResult?
        | none => pure ()
      return result?
