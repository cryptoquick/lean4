...
def synthInstanceCore? (type : Expr) (maxResultSize? : Option Nat := none) : MetaM (Option Expr) := do
  ...
  withInTypeClassResolution do
    ...
    let fvarNormEnabled := (← IO.getEnv "LEAN_NO_FVAR_NORM") != some "1"
    let normCtx? ← if kind matches .noMVars && fvarNormEnabled then SynthNorm.normalizeContext? cacheKeyType localInsts else pure none
    ...
    let baseKey := match normCtx? with
      | some c => { rawBaseKey with localInsts := c.canonLocalInsts, type := c.normType, normFVarTypes := c.fvarTypes }
      | none   => rawBaseKey
    ...
    if let some entry := sharedEntry? then
      ...
      match normCtx? with
      | none   => cacheResult key rel kind abstResult? result?
      | some c =>
        ...
        | some (nAbstResult?, nResult?) => cacheResult key rel kind nAbstResult? nResult?
        | none => pure ()
      return result?
      catch e =>
