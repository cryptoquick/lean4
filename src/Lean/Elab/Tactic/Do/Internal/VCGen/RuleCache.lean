...
Cache key: `(operator head, instWP, excessArgs.size)`.
-/
public def mkFrameBackwardRuleCached (op : Expr) (info : WPInfo) : VCGenM BackwardRule := do
  let opHead := op.getAppFn.constName?.getD .anonymous
  let key := (opHead, ExprPtr.mk info.instWP, info.excessArgs.size)
